...
function WineRow({ w, p, tweaks }) {
  const { showCost, showSupplier, showTags, compactRows, rule, bodySize } = tweaks;
  const rowPad = compactRows ? '6px 0' : '11px 0';

  return (
    <div style={{
      display: 'grid',
      gridTemplateColumns: `64px minmax(0, 1fr) ${showCost ? '80px ' : ''}90px ${showSupplier ? '120px' : ''}`.trim(),
      gap: 18,
      alignItems: 'baseline',
      padding: rowPad,
      ...
      {/* cost */}
      {showCost && (
        <div style={{ textAlign: 'right', color: p.muted, fontVariantNumeric: 'tabular-nums', fontSize: bodySize - 1 }}>
          {fmtPrice(w.priceZakup)}
        </div>
      )}
      ...
      {/* supplier */}
      {showSupplier && (
        <div style={{ color: p.muted, fontSize: bodySize - 1, textAlign: 'right' }}>
          {w.supplier}
        </div>
      )}
    </div>
...
// ---- Column header ----
function ColumnHeader({ p, tweaks }) {
  const { showCost, showSupplier, bodySize } = tweaks;
  const labels = ['Год', 'Наименование / Производитель'];
  if (showCost) labels.push('Закуп');
  labels.push('Бокал');
  if (showSupplier) labels.push('Поставщик');
  return (
    <div style={{
      display: 'grid',
      gridTemplateColumns: `64px minmax(0, 1fr) ${showCost ? '80px ' : ''}90px ${showSupplier ? '120px' : ''}`.trim(),
      ...
      <div>{labels[0]}</div>
      <div>{labels[1]}</div>
      {showCost && <div style={{ textAlign:'right' }}>Закуп ₽</div>}
      <div style={{ textAlign: 'right' }}>{showCost ? 'Бокал ₽' : 'Цена ₽'}</div>
      {showSupplier && <div style={{ textAlign: 'right' }}>Поставщик</div>}
    </div>
  );
}
...
function TopSection({ section, p, tweaks, idx }) {
  return (
    <section data-screen-label={`${String(idx+1).padStart(2,'0')} ${section.name}`} style={{ marginTop: idx === 0 ? 0 : 92, breakInside: 'avoid' }}>
      ...
      <ColumnHeader p={p} tweaks={tweaks} />
  ...
  return (
    <div style={{
      ...
      <TweaksPanel title="Tweaks · Винная карта">
        ...
        <TweakToggle label="Цена закупа" value={t.showCost} onChange={(v) => setT('showCost', v)} />
        <TweakToggle label="Поставщик" value={t.showSupplier} onChange={(v) => setT('showSupplier', v)} />
